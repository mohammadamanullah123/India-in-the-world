...
function isSecondPlotEnabled() {
    return true; // Always enable second plot on all devices
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (activeTab === 'raw-data') {
        ...
        if (!indiaDashboardLoaded) {
            await loadIndiaDashboard();
        }
    }
...
function renderMap() {
    ...
    svg.selectAll('.country')
        ...
        .on('click', function(event, d) {
            ...
            if (countryData && countryData.value !== null && !isNaN(countryData.value)) {
                const decimals = currentIndexInfo ? currentIndexInfo.decimals : 2;
                value = countryData.value.toLocaleString('en-US', {
                    minimumFractionDigits: decimals,
                    maximumFractionDigits: decimals
                });
            }
            
            if (window.openAIInsightPanel) {
                window.openAIInsightPanel({
                    country: countryName,
                    indicatorName: indexLabel,
                    value: value,
                    // We could potentially calculate rank here if needed, passing null for now
                    rank: null, 
                    totalCountries: null
                });
            }
...
async function initializeTimelineTab() {
    ...
    timelineTabState.selectedCountries = new Set(['IND', 'CHN', 'USA']);
...
/* ==========================================================================
   India Global Progress Dashboard Logic
   ========================================================================== */

let indiaDashboardLoaded = false;

// Hook into existing tab switching mechanism
document.addEventListener('click', (e) => {
    const tabBtn = e.target.closest('.tab-btn[data-tab="india"]');
    if (tabBtn) {
        // Tab switching is handled globally by existing tab logic,
        // we just need to load the content if it hasn't been loaded yet.
        if (!indiaDashboardLoaded) {
            loadIndiaDashboard();
        }
    }
...
async function loadIndiaDashboard() {
    indiaDashboardLoaded = true;
    const loader = document.getElementById('india-loader');
    const grid = document.getElementById('india-rankings-grid');
    if (loader) loader.classList.remove('hidden');

    try {
        // Fetch raw data for the current analysis year (e.g., 2023)
        // Check if there is data for 2023, otherwise try 2022
        let year = 2023;
        let response = await fetch(`/api/raw-data?year=${year}`);
        let data = await response.json();

        if (!data || data.length === 0) {
            year = 2022;
            response = await fetch(`/api/raw-data?year=${year}`);
            data = await response.json();
        }

        // Update year display
        const yearDisplay = document.getElementById('india-year-display');
        if (yearDisplay) yearDisplay.textContent = year;

        // Update indicators tracked count dynamically
        const countDisplay = document.querySelector('.india-hero-stats .india-stat:nth-child(2) .india-stat-value');
        if (countDisplay) {
            countDisplay.textContent = state.indexes.length;
        }

        // Group indicators by category from state.indexes (indicators.json)
        const categories = {};
        state.indexes.forEach(ind => {
        ...
        Object.keys(categories).sort().forEach(catName => {
            const indicators = categories[catName];

            let itemsHTML = '';
            let validCount = 0;
            let topIndicators = [];
            let bottomIndicators = [];

            indicators.forEach(ind => {
                // Calculate global rank for this indicator
                // Extract all valid values for this indicator
                const values = data.filter(d => d[ind.id] !== null && d[ind.id] !== undefined)
                    .map(d => ({ country: d.country_code, value: d[ind.id] }));

                // Sort values. Default to descending (higher is better). 
                // If it's a negative indicator (like debt, poverty, mortality), sort ascending.
                const isNegative = ind.scale && ind.scale.gradientClass && (ind.scale.gradientClass === 'negative' || ind.scale.gradientClass === 'inflation');

                values.sort((a, b) => {
                    return isNegative ? a.value - b.value : b.value - a.value;
                });

                const indiaIndex = values.findIndex(v => v.country === 'IND');
                let rankText = '--';
                let valueText = 'No Data';
                let rankClass = 'no-data';
                const totalCountries = values.length;
                const totalCountriesStr = totalCountries > 0 ? totalCountries : '--';

                validCount++; // Always increment so it renders

                if (indiaIndex !== -1) {
                    const rank = indiaIndex + 1;
                    rankText = `#${rank}`;
                    const indiaVal = values[indiaIndex].value;
                    valueText = formatAxisTick(indiaVal) + (ind.unit ? ' ' + ind.unit : '');

                    // Determine tier based on rank percentile
                    const percentile = rank / totalCountries;
                    if (percentile <= 0.33) {
                        rankClass = 'top-tier';
                        topIndicators.push(ind.label);
                    } else if (percentile <= 0.66) {
                        rankClass = 'mid-tier';
                    } else {
                        rankClass = 'bottom-tier';
                        bottomIndicators.push(ind.label);
                    }
                }

                itemsHTML += `
                    <div class="india-indicator-item">
                        <div class="india-indicator-info">
                            <div class="india-indicator-name" title="${ind.label}">${ind.label}</div>
                            <div class="india-indicator-value">${valueText}</div>
                        </div>
                        <div class="india-indicator-rank">
                            <span class="rank-badge ${rankClass}">${rankText}</span>
                            <span class="rank-total">/ ${totalCountriesStr}</span>
                        </div>
                    </div>
                `;
            });
            ...
            if (validCount > 0) {
                gridHTML += `
                    <div class="india-category-card">
                        <div class="india-category-header">
                            <h3 class="india-category-title">${catName}</h3>
                            <span class="india-category-count">${validCount} indicators</span>
                        </div>
                        <div class="india-indicator-list">
                            ${itemsHTML}
                        </div>
                    </div>
                `;
        ...
        console.error('Error loading India Dashboard:', error);
        if (grid) {
            grid.innerHTML = `<div class="error-msg" style="padding: 2rem; color: #dc3545;">Failed to load India data: ${error.message}</div>`;
        }
    } finally {
...
window.openAIInsightPanel = function(contextData) {
    const panel = document.getElementById('ai-insight-panel');
    const contextEl = document.getElementById('ai-insight-context');
    ...
    contextEl.innerHTML = `<strong>Country:</strong> ${contextData.country}<br><strong>Indicator:</strong> ${contextData.indicatorName}<br><strong>Value:</strong> ${contextData.value}`;
    ...
    btn.onclick = async () => {
        ...
        try {
            ...
            if (data.insight) {
                ...
                textEl.innerText = 'Could not generate insight: ' + (data.error || 'Unknown error');
            }
        } catch (e) {
            textEl.innerText = 'Error calling AI service. Is the backend running?';
        } finally {
            document.getElementById('ai-loader').classList.add('hidden');
            btn.disabled = false; // allow regenerating
        }
    };
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (contextEl && !contextEl.innerHTML) {
        contextEl.innerHTML = "Click a country on the map to select it.";
    }
