...
app.post('/api/insights', async (req, res) => {
  ...
  try {
    const { country, indicatorName, value, rank, totalCountries } = req.body;
    
    if (!country || !indicatorName) {
      return res.status(400).json({ error: 'Missing country or indicatorName in request body.' });
    }

    const prompt = `Act as a top-tier data analyst providing insights for a dashboard called "Data of the World". 
Analyze the following data point:
Country: ${country}
Indicator: ${indicatorName}
Current Value: ${value}
Current Rank: ${rank || 'Unknown'} out of ${totalCountries || 'Unknown'} globally.

Provide a 2-3 sentence insightful explanation of what might be driving this ranking or value for this country, and what it means in a global context. 
Keep it concise, professional, and easy to understand for a general audience. Do not use formatting like bolding or bullet points, just a short paragraph.`;
